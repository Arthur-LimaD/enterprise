...
export default function CompanyDataEdit ({companyData, setCompanyData, setMessage, setMessageType, setDates}) {
    ...
    function editCompany (e) {
        ...
        axios.put(`http://localhost:80/company/${id}/edit/data`, object)
        .then((res)=> {
            setCompanyData(res.data[0])
            setMessage('Data Added to Log With sucess!')
            setMessageType('success')
            setDates(res.data[0].data)
        })
        .catch((error)=> {
    ...
    return(<>
        ...
        (<div className={styles.CompanyEdit}>
                ...
                {/*Title*/ date ? (<h2>Edit Your Company's Information This Day!</h2>) 
